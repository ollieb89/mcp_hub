...
### Why aren't my tools being filtered?
...
**Fix:** Ensure server names match exactly (case-sensitive)

**Check 4: Mode Configuration**
```bash
curl http://localhost:37373/api/filtering/stats | jq '.mode'
# Expected: "server-allowlist", "category", or "hybrid"
```

**Fix:** Ensure mode matches your configuration

---

### Why are important tools missing?

**Symptom:** Specific tools you need are not exposed to clients

**Diagnostic:**
```bash
# Check if tool exists at all
curl http://localhost:37373/api/tools?includeFiltered=true | \
  jq '.tools[] | select(.name == "github__create_pr")'

# Check filtering stats
curl http://localhost:37373/api/filtering/stats | jq '.serverStats'
```

**Solution 1: Add Server to Allowlist**
```json
{
  "serverFilter": {
    "mode": "allowlist",
    "servers": ["filesystem", "github"]  // Add missing server
  }
}
```

**Solution 2: Add Category**
```json
{
  "categoryFilter": {
    "categories": [
...
```json
{
  "categoryFilter": {
    "categories": ["filesystem", "custom"],
    "customMappings": {
      "github__create_pr": "custom"  // Force include
    }
  }
}
```

**Solution 4: Switch to Hybrid Mode**
```json
{
  "mode": "hybrid",
  "serverFilter": {
    "mode": "allowlist",
    "servers": ["github"]  // Ensure critical servers
  },
  "categoryFilter": {
    "categories": ["filesystem", "version-control"]
  }
...
### Why is LLM categorization failing?

**Symptom:** LLM errors in logs, categorization falls back to 'other'
...
# Check logs
tail -f ~/.local/state/mcp-hub/logs/mcp-hub.log | grep -i "llm\|error"
...
# Should return models list, not 401 error
```

**3. Rate Limiting**
```bash
# Check for "Rate limit exceeded" in logs
tail -f ~/.local/state/mcp-hub/logs/mcp-hub.log | grep "rate limit"
...
```json
{
  "llmProvider": {
    "provider": "openai",
    "model": "gpt-4o-mini"  // Check spelling
  }
}
```
...
### How do I debug filtering issues?
...
**Step 2: Check Filtering Statistics**
...
```bash
# Check if specific tool is exposed
curl http://localhost:37373/api/tools | \
  jq '.tools[] | select(.name == "github__create_pr")'

# If not found, check with includeFiltered
curl http://localhost:37373/api/tools?includeFiltered=true | \
  jq '.tools[] | select(.name == "github__create_pr")'
```
...
# Verify filtering config
cat mcp.json | jq '.toolFiltering'
```

**Step 5: Review Server Registration**
...
curl http://localhost:37373/api/servers | \
  jq '.servers[] | {name, status, toolCount}'
```

**Step 6: Check Category Mappings**

```bash
# Debug logs will show categorization
tail -f ~/.local/state/mcp-hub/logs/mcp-hub.log | grep "category"

# Example output:
# [DEBUG] Tool 'github__create_pr' categorized as 'version-control'
# [DEBUG] Tool 'unknown__tool' categorized as 'other' (LLM)
```
